from playwright.async_api import Page, ElementHandle, ProxySettings, Locator
from asyncio import sleep, new_event_loop, set_event_loop
from PanelsHandler.utils.BrowserPooling import BrowserPool
from PanelsHandler.utils.logger import AsyncLogger
from datetime import datetime, UTC
import threading
import traceback
import inspect
import re


class UltraPandaPanel(object):
    """Unofficial API wrapper for the Ultra Panda panel handles the API calls and responses"""
    __LOGIN_URL: str = "https://ht.ultrapanda.mobi/#/login"
    __LANDING_PAGE: str = "https://ht.ultrapanda.mobi/#/index"
    __ACCOUNT_PAGE: str = "https://ht.ultrapanda.mobi/#/manage-user/account"
    __SCORE_PAGE: str = "https://ht.ultrapanda.mobi/#/manage-user/search"

    def __init__(
            self,
            headless: bool = True,
            storage_state_path: str = "PanelsHandler/PanelMemoryFiles/ultra_panda_storage.json",
            trained_captcha_solver_model_path: str = "PanelsHandler/TrainedModels/captcha_solver_v1.keras",
            panel_username: str = "DarkBytes",
            panel_password: str = "AbdulMoez@@5454",
            enable_js: bool = True,
            timeout: int = 30000,
            num_browsers: int = 2,
            logs_file: str = "PanelsHandler/PanelLogs/ultra_panda_logs.log",
            logs_backup_count: int = 7,
            max_browsers: int = 3,
            interval_minutes: int = 60,  # one hour
            proxy: any([ProxySettings, None]) = None,
    ) -> None:
        self.__timeout: int = timeout
        self.interval_minutes = interval_minutes
        self.stop_scheduler: bool = False
        self.browser_pool: BrowserPool = BrowserPool(
            login_page_url=self.__LOGIN_URL,
            landing_page_url=self.__LANDING_PAGE,
            num_browsers=num_browsers,
            max_browsers=max_browsers,
            storage_state_path=storage_state_path,
            trained_captcha_solver_model_path=trained_captcha_solver_model_path,
            panel_password=panel_password,
            panel_username=panel_username,
            enable_js=enable_js,
            timeout=timeout,
            headless=headless,
            proxy=proxy
        )
        self.__logger = AsyncLogger(
            class_name=self.__class__.__name__,
            log_file=logs_file,
            when="midnight",
            backup_count=logs_backup_count
        )

    async def initialize_panel(self):
        await self.__logger.logs(message='INFO: Initializing Panel.')
        await self.browser_pool.initialize_pool(ultra_panda_related=True)
        if self.stop_scheduler:
            self.stop_scheduler = True
            await sleep(delay=1)
            await self.start_scheduler()
        else:
            await self.start_scheduler()
        await self.__logger.logs(message='INFO: Panel Initialized successfully.')

    async def start_scheduler(self):
        """Start the scheduler in a separate thread."""
        thread = threading.Thread(target=self._scheduler_loop, daemon=True)
        thread.start()

    async def stop_scheduler_function(self):
        """Set flag to stop the scheduler."""
        self.stop_scheduler = True

    def _scheduler_loop(self):
        """Scheduler loop running every 30 minutes."""
        loop = new_event_loop()
        set_event_loop(loop)
        loop.run_until_complete(self._scheduler_async())
        loop.close()

    async def _scheduler_async(self):
        """Async function to call the initialization every `interval_minutes` minutes.

        This function checks the `self.stop_scheduler` flag every 0.5 seconds to determine
        if it should terminate early. If the scheduler is not stopped, it waits for the
        specified interval before executing the `agent_yolo_related_session` method.
        """
        sleep_seconds = self.interval_minutes * 60
        check_interval = 0.5
        while not self.stop_scheduler:
            elapsed = 0.0
            while elapsed < sleep_seconds and not self.stop_scheduler:
                remaining = sleep_seconds - elapsed
                current_sleep = min(check_interval, remaining)
                await sleep(current_sleep)
                elapsed += current_sleep
            if self.stop_scheduler:
                break
            try:
                await self.browser_pool.update_pool(ultra_panda_related=True)
            except Exception as e:
                print(f"Error: {str(e)}")

    async def ping_game_server(self) -> dict:
        return {
            "status": "active" if await self.browser_pool.ping_server(target_url=self.__LOGIN_URL) else "inactive",
            "pinged_url": self.__LOGIN_URL,
            "datetime": datetime.now(tz=UTC).__str__(),
        }

    @staticmethod
    async def __is_valid_username(username: str) -> bool:
        if 7 <= len(username) <= 16:
            return True
        else:
            return False

    @staticmethod
    async def __is_valid_password(password: str) -> bool:
        if not (6 <= len(password) <= 16):
            return False
        if not re.search(r"[A-Za-z]", password) or not re.search(r"\d", password):
            return False
        if not re.match(r"^[A-Za-z\d!@#$%^/.,()]+$", password):
            return False
        return True

    async def create_player(self, username: str, password: str, nickname: str = "") -> dict:
        """This function takes the username, password and nickname of the new user and crated it on the panel using
        the browse automation and returns True if user created otherwise False if we encounter any errors."""
        if not await self.__is_valid_username(username=username):
            return {
                'status': 400,
                'message': "Username must be between 7 and 16 characters long.",
                "data": {
                    "username": username,
                    "datetime": datetime.now(tz=UTC).__str__(),
                    "nickname": nickname,
                    "score": 0
                }
            }
        if not await self.__is_valid_password(password=password):
            return {
                'status': 400,
                'message': "Password length must be between 6-24 characters long. Must include a combination of "
                           "numbers and letters, and allows some special characters: !@#$%^/.,()",
                "data": {
                    "username": username,
                    "datetime": datetime.now(tz=UTC).__str__(),
                    "nickname": nickname,
                    "score": 0
                }
            }
        raw_response = await self.browser_pool.get_available_browser()
        browser_id: int = raw_response[-1]
        response = raw_response[0]
        browser_page: Page = response[2]
        try:
            await browser_page.goto(url=self.__ACCOUNT_PAGE, wait_until="domcontentloaded")
            await browser_page.locator(selector='span:text("Add Player")').click()
            username_input_field: ElementHandle = await browser_page.query_selector(
                selector='label[for="account"] + div input[type="text"]')
            await username_input_field.fill(value=username)
            password_input_field: ElementHandle = await browser_page.query_selector(
                selector='input[type="password"]')
            await password_input_field.fill(value=password)
            nickname_input_field: ElementHandle = await browser_page.query_selector(
                selector='label[for="name"] + div input[type="text"]')
            await nickname_input_field.fill(value=nickname)
            submit_button: ElementHandle = await browser_page.query_selector(
                selector='div[class$="m-btn el-form-item--medium"]  button'
            )
            await submit_button.click()
        except Exception as e:
            full_error = f"ERROR: {traceback.format_exc()}\n{e}"
            await self.browser_pool.release_browser(index=browser_id)
            await self.__logger.logs(
                message=f"ERROR: Unexpected error: {full_error}",
                function=inspect.currentframe().f_code.co_name
            )
            return {
                'status': 500,
                'message': "User not created. Please try again after few minutes.",
                "data": {
                    "username": username,
                    "datetime": datetime.now(tz=UTC).__str__(),
                    "nickname": nickname,
                    "score": 0
                }
            }
        await self.browser_pool.release_browser(index=browser_id)
        return {
            'status': 200,
            'message': "User Created successfully",
            "data": {
                "username": username,
                "datetime": datetime.now(tz=UTC).__str__(),
                "nickname": nickname,
                "score": 0
            }
        }

    async def get_panel_balance(self) -> dict:
        """get the balance available in the panel so that we can make sure in agent panel that this game gets
        enough balance"""
        raw_response = await self.browser_pool.get_available_browser()
        browser_id: int = raw_response[-1]
        response = raw_response[0]
        browser_page: Page = response[2]
        try:
            await browser_page.goto(url=self.__LANDING_PAGE, wait_until="domcontentloaded")
            await browser_page.locator(
                selector='i[class="el-icon-loading"][style="display: none;"]'
            ).locator('..').click()
            balance_item: ElementHandle = await browser_page.wait_for_selector(
                selector='i[class="el-icon-loading"][style="display: none;"] + span', timeout=6000
            )
            raw_balance = await balance_item.text_content()
            panel_balance = int(float(raw_balance.strip()))
        except Exception as e:
            full_error = f"ERROR: {traceback.format_exc()}\n{e}"
            await self.browser_pool.release_browser(index=browser_id)
            await self.__logger.logs(
                message=f"ERROR: Unexpected error: {full_error}",
                function=inspect.currentframe().f_code.co_name
            )
            return {
                'status': 500,
                'message': "Unable to get panel scores. Please try again after few minutes.",
                "data": {
                    "datetime": datetime.now(tz=UTC).__str__(),
                }
            }
        await self.browser_pool.release_browser(index=browser_id)
        return {
            'status': 200,
            'message': "Successfully fetched panel scores.",
            "data": {
                "datetime": datetime.now(tz=UTC).__str__(),
                'score': panel_balance
            }
        }

    async def reset_user_password(self, username: str, new_password: str) -> dict:
        """Reset the password if the user forget his/her password, and we can assign them new password"""
        if not await self.__is_valid_username(username=username):
            return {
                'status': 400,
                'message': "Invalid username.",
                "data": {
                    "username": username,
                    "newpassword": new_password,
                    "datetime": datetime.now(tz=UTC).__str__(),
                }
            }
        if not await self.__is_valid_password(password=new_password):
            return {
                'status': 400,
                'message': "Password length must be between 6-24 characters long. Must include a combination of"
                           " numbers and letters, and allows some special characters: !@#$%^/.,()",
                "data": {
                    "username": username,
                    "newpassword": new_password,
                    "datetime": datetime.now(tz=UTC).__str__(),
                }
            }
        raw_response = await self.browser_pool.get_available_browser()
        browser_id: int = raw_response[-1]
        response = raw_response[0]
        browser_page: Page = response[2]
        try:
            await browser_page.goto(url=self.__SCORE_PAGE, wait_until="domcontentloaded")
            await browser_page.locator(selector='span:text("Player account")').click()
            username_input_field: ElementHandle = await browser_page.query_selector(
                selector='div[role="combobox"] input[valuekey="value"]')
            await username_input_field.fill(value=username)
            submit_button: ElementHandle = await browser_page.query_selector(
                selector='div[class="el-form-item__content"]  button'
            )
            await submit_button.click()
            available_players: list[Locator] = await browser_page.locator(
                selector='div[class="m-mixed"] tr[class="el-table__row"]'
            ).all()
            await browser_page.wait_for_selector(
                selector='div[class="m-mixed"] tr[class="el-table__row"]', timeout=self.__timeout
            )
            found_player = False
            for player in available_players:
                player_username: str = await player.locator(selector_or_locator='td').nth(1).text_content()
                if player_username.strip() == username:
                    await player.locator(selector_or_locator='span:text("Edit")').click()
                    found_player = True
                    break
            if found_player:
                password_input_field: ElementHandle = await browser_page.query_selector(
                    selector='input[type="password"]'
                )
                await password_input_field.fill(value=new_password)
                submit_button: ElementHandle = await browser_page.wait_for_selector(
                    selector='#app > div > div.main-container.hasTagsView > section > div > div:nth-child(6) > '
                             'div > form > div.el-form-item.m-btn.el-form-item--medium > div > button',
                    timeout=self.__timeout
                )
                await submit_button.click()
                close_warning_popup: ElementHandle = await browser_page.wait_for_selector(
                    selector='div.el-message-box__btns >button.el-button.el-button--default.el-button'
                             '--small.el-button--primary',
                    timeout=self.__timeout
                )
                await close_warning_popup.click()
                await self.browser_pool.release_browser(index=browser_id)
                return {
                    'status': 200,
                    'message': "Password reset successfully.",
                    "data": {
                        "username": username,
                        "newpassword": new_password,
                        "datetime": datetime.now(tz=UTC).__str__(),
                    }
                }
            else:
                await self.browser_pool.release_browser(index=browser_id)
                return {
                    'status': 422,
                    'message': "Unable to find user. Please create user first.",
                    "data": {
                        "username": username,
                        "newpassword": new_password,
                        "datetime": datetime.now(tz=UTC).__str__(),
                    }
                }
        except Exception as e:
            full_error = f"ERROR: {traceback.format_exc()}\n{e}"
            await self.browser_pool.release_browser(index=browser_id)
            await self.__logger.logs(
                message=f"ERROR: Unexpected error: {full_error}",
                function=inspect.currentframe().f_code.co_name
            )
            return {
                'status': 422,
                'message': "Unable to reset the password.",
                "data": {
                    "username": username,
                    "newpassword": new_password,
                    "datetime": datetime.now(tz=UTC).__str__(),
                }
            }

    async def redeem_user_score(self, username: str, score: int) -> dict:
        """If the user want to get the score into money we will redeem score and add money to their wallet"""
        if not await self.__is_valid_username(username=username):
            return {
                'status': 400,
                'message': "Invalid username.",
                "data": {
                    "username": username,
                    "datetime": datetime.now(tz=UTC).__str__(),
                }
            }
        raw_response = await self.browser_pool.get_available_browser()
        browser_id: int = raw_response[-1]
        response = raw_response[0]
        browser_page: Page = response[2]
        try:
            await browser_page.goto(url=self.__SCORE_PAGE, wait_until="domcontentloaded")
            await browser_page.reload(wait_until="domcontentloaded")
            await browser_page.locator(selector='span:text("Player account")').click()
            username_input_field: ElementHandle = await browser_page.query_selector(
                selector='div[role="combobox"] input[valuekey="value"]')
            await username_input_field.fill(value=username)
            submit_button: ElementHandle = await browser_page.query_selector(
                selector='div[class="el-form-item__content"]  button'
            )
            await submit_button.click()
            await browser_page.wait_for_selector(
                selector='div[class="m-mixed"] tr[class="el-table__row"]', timeout=self.__timeout
            )
            available_players: list[Locator] = await browser_page.locator(
                selector='div[class="m-mixed"] tr[class="el-table__row"]'
            ).all()
            found_player = False
            current_score = 0
            for player in available_players:
                player_username: str = await player.locator(selector_or_locator='td').nth(1).text_content()
                if player_username.strip() == username:
                    current_raw_scores = await player.locator(
                        selector_or_locator="td"
                    ).nth(6).text_content()
                    current_score = int(current_raw_scores.strip())
                    await player.locator(selector_or_locator='span:text("Set Score")').click()
                    found_player = True
                    break
            if found_player:
                score_removal = score * -1
                if current_score >= score:
                    score_input_field: ElementHandle = await browser_page.wait_for_selector(
                        selector='label[for="score"] + div input[type="text"]', timeout=self.__timeout
                    )
                    await score_input_field.fill(value=str(score_removal))
                    await browser_page.locator(selector='span:text("OK")').nth(1).click()
                    player_credits = current_score + score_removal
                    await self.browser_pool.release_browser(index=browser_id)
                    return {
                        'status': 200,
                        'message': "Score successfully redeemed scores from the game.",
                        "data": {
                            "username": username,
                            "new_scores": player_credits,
                            "datetime": datetime.now(tz=UTC).__str__(),
                        }
                    }
                else:
                    await self.browser_pool.release_browser(index=browser_id)
                    return {
                        'status': 422,
                        'message': "You don't have enough scores to redeem.",
                        "data": {
                            "username": username,
                            "current_score": int(current_score),
                            "datetime": datetime.now(tz=UTC).__str__(),
                        }
                    }
            else:
                await self.browser_pool.release_browser(index=browser_id)
                return {
                    'status': 422,
                    'message': "Unable to find user. Please create a user first.",
                    "data": {
                        "username": username,
                        "datetime": datetime.now(tz=UTC).__str__(),
                    }
                }
        except Exception as e:
            full_error = f"ERROR: {traceback.format_exc()}\n{e}"
            await self.browser_pool.release_browser(index=browser_id)
            await self.__logger.logs(
                message=f"ERROR: Unexpected error: {full_error}",
                function=inspect.currentframe().f_code.co_name
            )
            return {
                'status': 422,
                'message': "Unable to redeem scores from the game.",
                "data": {
                    "username": username,
                    "datetime": datetime.now(tz=UTC).__str__(),
                }
            }

    async def add_user_score(self, username: str, score: int) -> dict:
        """We can assign them more scores and deduct the money from their wallets"""
        if not await self.__is_valid_username(username=username):
            return {
                'status': 400,
                'message': "Invalid username.",
                "data": {
                    "username": username,
                    "datetime": datetime.now(tz=UTC).__str__(),
                }
            }
        panel_scores = await self.get_panel_balance()
        if 'score' in panel_scores['data']:
            if not panel_scores['data']['score'] >= score:
                return {
                    'status': 500,
                    'message': "Not enough Panel scores please contact our agents..",
                    "data": {
                        "username": username,
                        "datetime": datetime.now(tz=UTC).__str__(),
                    }
                }
        else:
            return {
                'status': 500,
                'message': "Unable to get Panel scores please contact our agents.",
                "data": {
                    "username": username,
                    "datetime": datetime.now(tz=UTC).__str__(),
                }
            }
        raw_response = await self.browser_pool.get_available_browser()
        browser_id: int = raw_response[-1]
        response = raw_response[0]
        browser_page: Page = response[2]
        try:
            await browser_page.goto(url=self.__SCORE_PAGE, wait_until="domcontentloaded")
            await browser_page.reload(wait_until="domcontentloaded")
            await browser_page.locator(selector='span:text("Player account")').click()
            username_input_field: ElementHandle = await browser_page.query_selector(
                selector='div[role="combobox"] input[valuekey="value"]')
            await username_input_field.fill(value=username)
            submit_button: ElementHandle = await browser_page.query_selector(
                selector='div[class="el-form-item__content"]  button'
            )
            await submit_button.click()
            await browser_page.wait_for_selector(
                selector='div[class="m-mixed"] tr[class="el-table__row"]', timeout=self.__timeout
            )
            available_players: list[Locator] = await browser_page.locator(
                selector='div[class="m-mixed"] tr[class="el-table__row"]'
            ).all()
            found_player = False
            current_score = 0
            for player in available_players:
                player_username: str = await player.locator(selector_or_locator='td').nth(1).text_content()
                if player_username.strip() == username:
                    current_raw_scores = await player.locator(
                        selector_or_locator="td"
                    ).nth(6).text_content()
                    current_score = int(current_raw_scores.strip())
                    await player.locator(selector_or_locator='span:text("Set Score")').click()
                    found_player = True
                    break
            if found_player:
                score_input_field: ElementHandle = await browser_page.wait_for_selector(
                    selector='label[for="score"] + div input[type="text"]', timeout=self.__timeout
                )
                await score_input_field.fill(value=str(score))
                await browser_page.locator(selector='span:text("OK")').nth(1).click()
                player_credits = current_score + score
                await self.browser_pool.release_browser(index=browser_id)
                return {
                    'status': 200,
                    'message': "Score successfully added to the game.",
                    "data": {
                        "username": username,
                        "new_scores": player_credits,
                        "datetime": datetime.now(tz=UTC).__str__(),
                    }
                }
            else:
                await self.browser_pool.release_browser(index=browser_id)
                return {
                    'status': 422,
                    'message': "Unable to find user. Please create user first.",
                    "data": {
                        "username": username,
                        "datetime": datetime.now(tz=UTC).__str__(),
                    }
                }
        except Exception as e:
            full_error = f"ERROR: {traceback.format_exc()}\n{e}"
            await self.browser_pool.release_browser(index=browser_id)
            await self.__logger.logs(
                message=f"ERROR: Unexpected error: {full_error}",
                function=inspect.currentframe().f_code.co_name
            )
            return {
                'status': 422,
                'message': "Unable to add score to the game.",
                "data": {
                    "username": username,
                    "datetime": datetime.now(tz=UTC).__str__(),
                }
            }

    async def get_user_scores(self, username: str) -> dict:
        """Get the current score of the user"""
        if not await self.__is_valid_username(username=username):
            return {
                'status': 400,
                'message': "Invalid username.",
                "data": {
                    "username": username,
                    "datetime": datetime.now(tz=UTC).__str__(),
                }
            }
        raw_response = await self.browser_pool.get_available_browser()
        browser_id: int = raw_response[-1]
        response = raw_response[0]
        browser_page: Page = response[2]
        try:
            await browser_page.goto(url=self.__SCORE_PAGE, wait_until="domcontentloaded")
            await browser_page.reload(wait_until="domcontentloaded")
            await browser_page.locator(selector='span:text("Player account")').click()
            username_input_field: ElementHandle = await browser_page.query_selector(
                selector='div[role="combobox"] input[valuekey="value"]')
            await username_input_field.fill(value=username)
            submit_button: ElementHandle = await browser_page.query_selector(
                selector='div[class="el-form-item__content"]  button'
            )
            await submit_button.click()
            await browser_page.wait_for_selector(
                selector='div[class="m-mixed"] tr[class="el-table__row"]', timeout=self.__timeout
            )
            available_players: list[Locator] = await browser_page.locator(
                selector='div[class="m-mixed"] tr[class="el-table__row"]'
            ).all()
            found_player = False
            current_score = 0
            for player in available_players:
                player_username: str = await player.locator(selector_or_locator='td').nth(1).text_content()
                if player_username.strip() == username:
                    current_raw_scores = await player.locator(
                        selector_or_locator="td"
                    ).nth(6).text_content()
                    current_score = int(current_raw_scores.strip())
                    found_player = True
                    break
            if found_player:
                await self.browser_pool.release_browser(index=browser_id)
                return {
                    'status': 200,
                    'message': "Score successfully fetch from the game.",
                    "data": {
                        "username": username,
                        "score": int(current_score),
                        "datetime": datetime.now(tz=UTC).__str__(),
                    }
                }
            else:
                await self.browser_pool.release_browser(index=browser_id)
                return {
                    'status': 422,
                    'message': "Unable to find user. Please create user first.",
                    "data": {
                        "username": username,
                        "datetime": datetime.now(tz=UTC).__str__(),
                    }
                }
        except Exception as e:
            full_error = f"ERROR: {traceback.format_exc()}\n{e}"
            await self.browser_pool.release_browser(index=browser_id)
            await self.__logger.logs(
                message=f"ERROR: Unexpected error: {full_error}",
                function=inspect.currentframe().f_code.co_name
            )
            return {
                'status': 500,
                'message': "Unable to find user. Please try again after few minutes.",
                "data": {
                    "username": username,
                    "datetime": datetime.now(tz=UTC).__str__(),
                }
            }

