...
class JuwaPanel(object):
    """Unofficial API wrapper for the Juwa panel handles the API calls and responses"""
    __LOGIN_URL: str = "https://ht.juwa777.com/login"
    __LANDING_PAGE: str = "https://ht.juwa777.com/HomeDetail"
    __PANEL_SCORE_API: str = "https://ht.juwa777.com/api/agent/balance"
    __ADD_PLAYER_API: str = "https://ht.juwa777.com/api/user/addUser"
    __USER_SCORE_API: str = "https://ht.juwa777.com/api/user/userList"
    __ADD_SCORE_API: str = "https://ht.juwa777.com/api/user/rechargeRedeem"
    __REDEEM_SCORE_API: str = "https://ht.juwa777.com/api/user/rechargeRedeem"
    __RESET_PASSWORD_API: str = "https://ht.juwa777.com/api/user/resetUserPwd"

    def __init__(
            self,
            headless: bool = True,
            storage_state_path: str = "PanelsHandler/PanelMemoryFiles/juwa_storage.json",
            trained_captcha_solver_model_path: str = "PanelsHandler/TrainedModels/captcha_solver_royal_0.1.keras",
            panel_username: str = "darkbytes01",
            panel_password: str = "AbdulMoez@@5454",
            enable_js: bool = True,
            timeout: int = 10000,
            num_browsers: int = 2,
            logs_file: str = "PanelsHandler/PanelLogs/juwa_logs.log",
    ...
    async def initialize_panel(self):
        ...
        if not await self.browser_pool.game_vault_related_session(
                username_place_holder='Please enter your account',
                password_place_holder='Please enter your password',
                is_juwa_panel=True
        ):
            await self.__logger.logs(
                message="ERROR: Juwa session does not created.",
    ...
    async def create_player(self, username: str, password: str, nickname: str = "") -> dict:
        ...
        agent_id = json.loads(others['user'])['agent_id']
        payloads = {
            'account': username,
            'nickname': nickname,
            'login_pwd': password,
        ...
        async with aiohttp.ClientSession(
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    if 'code' not in json_data or not json_data['code'] == 200:
                        await self.__logger.logs(
                    ...
                    for user in json_data['data']['list']:
                        if user['login_name'] == username:
                            ...
                            if current_user_balance >= score:
                                payloads = {
                                    'account': json.loads(others['user'])['login_name'],
                                    'amount': str(score),
