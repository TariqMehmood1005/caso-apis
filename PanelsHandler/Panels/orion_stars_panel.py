...
class OrionStarsPanel(object):
    """Unofficial API wrapper for the orion star panel handles the API calls and responses"""
    __LOGIN_URL: str = "https://orionstars.vip:8781/default.aspx"
    __LANDING_PAGE: str = "https://orionstars.vip:8781/Cashier.aspx"

    def __init__(
            self,
            headless: bool = True,
            storage_state_path: str = "PanelsHandler/PanelMemoryFiles/orion_stars_storage.json",
            ...
            enable_js: bool = True,
            timeout: int = 30000,
            logs_file: str = "PanelsHandler/PanelLogs/orion_stars_logs.log",
    ...
    async def create_player(self, username: str, password: str, nickname: str = "") -> dict:
        ...
        if not await self.__is_valid_username(username=username):
            return {
                'status': 400,
                'message': "Username must be 13 characters or fewer. It must include letters, underscores, and numbers.",
                "data": {
                    "username": username,
                    "datetime": datetime.now(tz=UTC).__str__(),
                    "nickname": nickname,
    ...
    async def reset_user_password(self, username: str, new_password: str) -> dict:
        ...
        try:
            ...
            await player_iframe.locator('div[class="SearchBg2"] > table tr[class="list"]'
            ...
            for player in available_players:
                ...
                if player_username.strip() == username:
                    ...
                    break
            if found_player:
                await player_iframe.locator(selector_or_locator='a[onclick*="Reset Password"]'
                ...
                await treasure_iframe.locator(selector_or_locator='input[name="txtConfirmPass"]'
                                              ).wait_for(state="visible")
                await treasure_iframe.locator(selector_or_locator='input[name="txtConfirmPass"]'
    ...
    async def redeem_user_score(self, username: str, score: int) -> dict:
        """If the user wants to get the score into money, we will redeem the score and add money to their wallet"""
        if not await self.__is_valid_username(username=username):
            return {
                'status': 400,
                'message': "Invalid username.",
