...
class MilkyWayPanel(object):
    """Unofficial API wrapper for the Milky Way panel handles the API calls and responses"""
    __LOGIN_URL: str = "https://milkywayapp.xyz:8781/default.aspx"
    __LANDING_PAGE: str = "https://milkywayapp.xyz:8781/Cashier.aspx"

    def __init__(
            self,
            headless: bool = True,
            storage_state_path: str = "PanelsHandler/PanelMemoryFiles/milky_way_storage.json",
            ...
            enable_js: bool = True,
            logs_file: str = "PanelsHandler/PanelLogs/milky_way_logs.log",
    ...
    async def get_panel_balance(self) -> dict:
        """get the balance available in the panel so that we can make sure in an agent panel that this game gets
        enough balances"""
        raw_response = await self.browser_pool.get_available_browser()
        browser_id: int = raw_response[-1]
        response = raw_response[0]
